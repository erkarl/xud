...
describe('MatchingEngine.removePeerOrders', () => {
  ...
  it('should add a new peerOrder and then not allow other peer to remove it', () => {
    const quantity = 5;
    ...
    expect(() => engine.removePeerOrder(
      ...
      otherPeerPubKey,
      quantity,
    )).to.throw(`peer order ${order.id} of peer ${otherPeerPubKey} could not be found`);
  });
...
describe('MatchingEngine queues and lists integrity', () => {
  ...
  it('queue and list should both have the maker order removed after a full match', () => {
    ...
    )).to.throw(`peer order ${peerOrder.id} of peer ${peerOrder.peerPubKey} could not be found`);
    const queueRemainingOrder = engine.queues.sell.peek();
